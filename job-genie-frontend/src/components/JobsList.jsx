...
var JobsList = ({
  ...
  const [jobbs, setJobs]=useState(jobs);
  ...
  useEffect(()=>{
    async function fetchApplications() {
      ...
      const Applications = applicationsResponse.data.filter((application) =>
      application.email == userData?.email
      );
    ...
    <div className="text-white">
      ...
      <div className="my-6 flex flex-col gap-6">
        {jobbs.map((job) => (
          <div
            key={job.id}
            className="my-4 p-4 flex justify-between items-center gap-4 border rounded-lg bg-sky-800 hover:bg-sky-700 hover:shadow-lg hover:scale-105 transform transition-transform duration-300"
          >
            <div>
            ...
            {isRecruiter && userData.jobIds.includes(job.id) ? (
              ...
              <div>
                <button
                  ...
                  disabled={isRecruiter||job.isapplied}
