...
const PostJobForm = () => {
  ...
  const validateForm = () => {
    ...
    if (!/^[a-zA-Z\s]+$/.test(position)) {
      setError("Position should only contain alphabetic characters.");
      isValid = false;
    } else if (!/^\d+$/.test(experience) || parseInt(experience) < 0) {
      setError("Experience should be a non-negative number.");
      isValid = false;
    } else if (!position || !company || !location || !experience || !description || skills.length === 0) {
  ...
  return (
    <form
      ...
      className="p-14 mb-24 bg-slate-700 w-full max-w-3xl 2xl:max-w-5xl rounded-lg flex flex-col gap-4 2xl:gap-10 mx-auto"
    >
